...
<html>
  <head>
    ...
    <title>Daale - Registro Usuario</title>
    ...
    <section>
      <div class="logo">
        ...
        <form class="animated fadeInUp" action="" id="formulario" method="post">
		       		<h2 class="">Regístrate como Usuario General</h2>
              <div class="line"></div>
		          	<input type="text" name="nombre" placeholder="Nombre" style="width:59%; margin-right:2%; float:left;" required>
                <input type="text" name="user" placeholder="Nombre de usuario" style="width:39%; float:left;" required>
                ...
                <input type="float" name="altura" placeholder="Altura" style="float:left; width:18%; height:38px;"  required>
                <select name="unidad" style="width:16%; float:left; margin-right:2%; height:38px;" class="select">
									<option value="m" class="dentro-select" required>m</option>
									<option value="cm" class="dentro-select">cm</option>
								</select>
                <input type="number" name="peso_act" placeholder="Peso Actual(kg)" style="width:32%; float:left;  margin-right:2%; height:38px;" required>
                <input type="number" name="peso_meta" placeholder="Peso Meta(kg)" style="width:30%; float:left; height:38px;" required>
                  ...
                  <?php
                		include("../../php/config.php");
        						...
        						$resultado2 = $conexion->query($query2);
        					 	while($row2 = $resultado2->fetch_assoc()){
        					?>
        					<option value="<?php echo $row2['ID_Preguntas']; ?>" class="dentro-select" require><?php echo $row2['Pregunta']; ?></option>
        				<?php } ?>
								</select>
                <input type="text" name="respuesta2" placeholder="" style="width:49%; float:left; height:40px;" required>
         ...
         <script type="text/javascript">
           jQuery(document).on('submit', '#formulario', function(event){
             ...
             .done(function(respuesta){
               ...
               if (!respuesta.error1 && !respuesta.error2 && !respuesta.error3  && !respuesta.error4 && !respuesta.error5) {
                 if (respuesta.var == 1) {
                 ...
                 $('.btn').val('Continuar');
               }else if (respuesta.error4 == true) {
                 $('#alerta2').slideDown('slow');
                 setTimeout(function(){
                     $('#alerta2').slideUp('slow');
                 }, 3000);
                 $('.btn').val('Continuar');
               }else if (respuesta.error5 == true) {
