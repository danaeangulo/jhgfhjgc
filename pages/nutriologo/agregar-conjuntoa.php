...
<head>
	...
	<title>Daale - Selección ingredientes</title>
...
<body>
	...
	<?php
		if ($_SESSION['ConjuntoA']==0) {
		}else if ($_SESSION['ConjuntoA']==2) {
			echo "<script>";
			...
			echo "</script>";
				$_SESSION['ConjuntoA']=0;
		}else if ($_SESSION['ConjuntoA']==3) {
			echo "<script>";
  ...
  <section style="padding-bottom:100px;">
		<div class="caja animated fadeInUp"  >
			...
			$sql0=("SELECT COUNT(ID_Alimentos) FROM conjuntoa WHERE ID_Recetas='$id_s'");
			$query0=mysqli_query($mysqli,$sql0);
			while($fila0=mysqli_fetch_array($query0)){
			$c=$fila0[0];
			}
			if ($c==0) {
				?> <h2>Agrega un ingrediente a tu receta</h2> <?php
			}else{
				?> <h2>Agrega otro ingrediente a tu receta</h2> <?php
			}
			?>
			<div class="line"></div>


		<form action="../../php/nutriologo/agregar-conjuntoa.php?id=<?php echo $id_s; ?>" method="post">
				<div class="text-left">Ingrediente:</div>
				...
				<div class="abajo-botones">
					<div class="cancelar2" style="width:auto;">
						<a  href="recetas.php">Finalizar receta</a>
					</div>
					<input type="submit" value="Agregar ingrediente" id="boton" style="width:auto; margin-right:10px; margin-left:5px;">
				</div>

				<div class="text-left" style="text-align:justify;">
					Aquí podrás seleccionar los ingredientes de tu receta.
				</div>
