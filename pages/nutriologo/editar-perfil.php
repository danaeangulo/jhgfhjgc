...
<body>
	...
	<div class="modal" id="clave">
		<div class="modal-contenido">
			...
			<form action="" id="form-clave-n" method="post">
			...
			<script type="text/javascript">
				jQuery(document).on('submit', '#form-clave-n', function(event){
					event.preventDefault();
					jQuery.ajax({
						url: '../../php/nutriologo/editar-clave.php',
						type: 'POST',
						dataType: 'json',
						data: $(this).serialize(),
						beforeSend: function(){
  ...
  <section>
		...
		<form action="" id="formulario" method="post">
				...
				<div class="text-left2">Apellido Paterno</div>
				<div class="text-left2">Apellido Materno</div>
				<input class="medio1" type="text" name="nombre" value="<?php echo $row['Nombre']; ?>" required>
				<input class="medio2" type="text" name="ap_p" value="<?php echo $row['Ap_P']; ?>" required>
				<input class="medio3" type="text" name="ap_m" value="<?php echo $row['Ap_M']; ?>" required>

				<div class="text-left">Fecha de nacimiento</div>
				<input  type="date" name="fecha" value="<?php echo $row['Fech_Nac']; ?>" required>


				<div class="text-left">Correo y teléfono</div>
				<input class="pequeño1" type="text" name="correo" value="<?php echo $row['Correo']; ?>" required>
				<input class="pequeño2" type="tel" name="telefono" value="<?php echo $row['Telefono']; ?>" required>
		...
		<script type="text/javascript">
			jQuery(document).on('submit', '#formulario', function(event){
				event.preventDefault();
				jQuery.ajax({
					url: '../../php/nutriologo/editar-perfil.php',
					type: 'POST',
					dataType: 'json',
					data: $(this).serialize(),
					beforeSend: function(){
